...
class Financeiro_model extends CI_Model
{
    ...
    public function getTotals($where = '')
    {
        ...
        $this->db->select("
            SUM(case when idLoja =  '$idLojaF' and tipo = 'despesa' then valor end) as despesas,
            SUM(case when idLoja =  '$idLojaF' and tipo = 'receita' then valor end) as receitas
        ");
    ...
    public function autoCompleteClienteFornecedor($q) 
     {
        $this->db->select('DISTINCT(cliente_fornecedor) as cliente_fornecedor,  idLoja');
        $this->db->limit(5);
        $this->db->like('cliente_fornecedor', $q);
        $query = $this->db->get('lancamentos');
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                if($row['idLoja'] == $this->session->userdata('idLoja')) { 
    ...
    {
        $this->db->select('idClientes, nomeCliente, idLoja');
        $this->db->limit(5);
